...
from vision_utils.generic_utils import dict_to_param_str,str2bool
...
def re_detect2(img, preds, min_score = 0,max_score=1,inflation_factor=1,is_abs=False,
  ...
  masked_images = []
  new_dets_map = {}
  for ibox,score in tqdm_notebook(enumerate(scores),total=len(preds)):
    bb = preds[[ibox]]
    ...
    if len(preds_masked) > 0:      
      ...
      preds_masked = preds_masked[ find((ious >= min_keep_iou).numpy().ravel())]    
      if len(preds_masked) > 0:      
        new_dets_map[ibox] = dict(source_ind = ibox,
                                  source=bb,result=preds_masked,
                                  result_ious = ious[0,ious_above_thresh],
                                  min_score=min_score,
...
def masked_detection_2(coco_dataset,coco_demo,args):
	...
	for i in tqdm(range(N)):
		...
		p = os.path.join(D, '{}.pkl'.format(k))
		cur_res = pickle.load(open(p, 'rb'))
		cur_res = remove_masks(cur_res)
		idx, k_, pred = cur_res['idx'], cur_res['k'], cur_res['preds']
		img, target, idx = coco_dataset[i]
		image = np.array(img)
		new_dets_map = re_detect2(img,cur_res['preds'], min_score = 0, max_score=1, inflation_factor=args.inflation_factor,
...
def areas_in_mask(preds,M):
	...
	return skimage.transform.integrate(M_int,starts,ends),boxutils.bb_areas(bb)
